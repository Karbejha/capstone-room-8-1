...
async function addNewRestaurant(req, res) {
  try {
    ...
    res.status(201).json({ message: "Restaurant added sucessfully", Restaurant: newRestaurant });
  } catch (error) {
...
const getRestaurant = async (_, res) => {
  try {
      ...
      const lstRestaurant = await RestaurantSchema.find({ });
           ...
           res.status(200).json({ message: 'List of Restaurant Results', Restaurant: lstRestaurant });
} 
catch (err) {
   res.status(422).json({ message: 'Error while getting Restaurant list', error: err.message });
}
}
